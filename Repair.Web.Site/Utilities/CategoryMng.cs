...
namespace Repair.Web.Site.Utilities
{
    public class CategoryMng
    {
        ...
        public static string UseCompanyId = null;
        public static UserType RoleKey;
                ...
                if (HttpRuntime.Cache.Get(_categoryTreeCacheKey) == null)
                {
                    lock (typeof(CategoryMng))
                    {
                        if (HttpRuntime.Cache.Get(_categoryTreeCacheKey) == null)
                        {
                            ...
                            {
                                //获取已有的设备类目集合
                                if (RoleKey.HasFlag(UserType.SuperAdmin)) {
                                    _categoryTree = db.Category.ToList();
                                }
                                else { 
                                    var deviceCategory =db.Device.Where(t => t.UseCompanyId == UseCompanyId)
                                                                    .GroupBy(t => t.CategoryId)
                                                                    .Select(t => t.Key).ToArray();
                                    _categoryTree = db.Category.Where(t=>deviceCategory.Contains(t.CategoryId)).ToList();
                                }
                            }
                    ...
                    //DOTO绝对过期， 此刻起时间超过3分钟立即过期
                    HttpRuntime.Cache.Insert(_categoryTreeCacheKey, _categoryTree, null, DateTime.Now.AddSeconds(60 * 1), System.Web.Caching.Cache.NoSlidingExpiration);
                }
